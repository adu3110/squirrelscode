...
const ProblemDescription: React.FC<ProblemDescriptionProps> = ({problem, _solved}) => {
	...
	const returnUserDataAndProblemData = async (transaction: any) => {
		...
		const problemRef = doc(firestore, "arraysAndHashingProblems", problem.id);
	...
	return (
		<div className='bg-sky-950'>
			{/* TAB */}
			<div className='flex h-11 w-full items-center pt-2 bg-sky-950 text-white overflow-x-hidden'>
				<div className={"bg-dark-fill-3 rounded-t-[5px] px-5 py-[10px] text-xs cursor-pointer"}>
					Description
				</div>
			</div>
...
function useGetCurrentProblem(problemId: string) {
	...
	useEffect(() => {
		...
		const getCurrentProblem = async () => {
			setLoading(true);
			const docRef = doc(firestore, "arraysAndHashingProblems", problemId);
