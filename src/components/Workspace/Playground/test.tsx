...
const javascriptDefault: string = `/**
* Problem: Binary Search: Search a sorted array for a target value.
*/

// Time: O(log n)
const binarySearch = (arr: number[], target: number): number | boolean => {
 return binarySearchHelper(arr, target, 0, arr.length - 1);
};

const binarySearchHelper = (arr: number[], target: number, start: number, end: number): number | boolean => {
 if (start > end) {
   return false;
 }
